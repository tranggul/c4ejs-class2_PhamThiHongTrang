let quote = [{
    ...
    let x = document.getElementById('quo');
    let y = document.getElementById('author');
    let z = document.getElementById('reload');
    function randQuote(){
        var rand = quote[Math.floor(Math.random() * quote.length)];
    
        x.innerHTML= rand.quoteText;
        y.innerHTML = rand.quoteAuthor;
    }
   z.addEventListener('click',randQuote);
